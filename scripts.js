...
document.getElementById(li.id).getElementsByTagName("A")[0].setAttribute('ondblclick','load_view_node(this.parentNode.id);'); obj1.parentNode.parentNode.style.visibility = 'hidden'; var pimg = document.getElementById(parentid).getElementsByTagName("IMG")[0]; pimg.style.visibility="visible"; pimg.src = pimg.src.replace(JSTreeObj.plusImage,JSTreeObj.minusImage); document.getElementById(parentid).getElementsByTagName("UL")[0].style.display="block"; treeObj.initTree();}, complete: function(ar1,ar2){ document.getElementById("iconIMGnode" + next_id).src = DRUPAL_PATH + '/' + ary[2]; update_weights();}
});}
}
...
$(document).ready(function(){ treeObj = new JSDragDropTree(); treeObj.setTreeId("node0"); treeObj.setMaximumDepth(100); treeObj.initTree(); get_book_roots(); load_outline(document.getElementById("selected_outline").value); treeObj.createDropIndicator(); document.documentElement.onselectstart = treeObj.cancelSelectionEvent; document.documentElement.ondragstart = treeObj.cancelEvent; document.documentElement.onmousemove = treeObj.moveDragableNodes; document.documentElement.onmouseup = treeObj.dropDragableNodes; treeObj.helpObj = document.createElement("DIV"); treeObj.helpObj.style.display = "none"; document.body.appendChild(treeObj.helpObj);}); function scale_outline_designer(scale){ if(scale == 'up' && factor != 2.5){ factor = factor + .25;}else if(scale == 'down' && factor != .5){ factor = factor - .25;}else if(scale == 'reset'){ factor = 1;}
if(factor == 1){ $("#node0 img").css('width','').css('height',''); $("#node0 a").css('font-size','');}else{ $("#node0 img").css('width',factor + 'em').css('height',factor + 'em'); $("#node0 a").css('font-size',factor + 'em');}
$("#node0 span").css('margin','0px ' + factor * 5 + 'px 0px ' + factor * 5 + 'px').css('padding','0px ' + factor * 5 + 'px ' + factor * 10 + 'px ' + factor * 5 + 'px'); $("#node0 span").css('background-image','url(' + DRUPAL_OD_PATH + '/images/settings' + (factor*100) + '.png)'); $("#dragDropIndicatorImage").css('padding',factor * 9 + 'px 0px 0px 0px').css('width',factor + 'em').css('height',factor + 'em');}
function load_outline(nid){ document.getElementById("node0").getElementsByTagName("UL")[0].innerHTML=""; if(nid !=0){ $.ajax({ type: "GET", url: AJAX_URL + "load_tree/" + nid, success: function(msg){ var ary = Array(); ary = PHP_Unserialize(msg); for(var i=0; i<ary.length; i++){ ary_nid = ary[i][0]; ary_pid = ary[i][1]; ary_title = ary[i][2]; ary_edit = ary[i][4]; nodename = "node" + ary_nid; var li = document.createElement("LI"); li.id = nodename; var span = document.createElement("SPAN"); span.id = "nodeLevel" + ary_nid; var a = document.createElement("A"); a.href = "#"; a.innerHTML = ary_title; var ul = document.createElement("UL"); li.appendChild(span); li.appendChild(a); li.appendChild(ul); parentid = "node" + ary_pid; document.getElementById("node0").getElementsByTagName("UL")[0].appendChild(li); if(OUTLINE_POSTS == 1){ document.getElementById(nodename).setAttribute("noadd","false");}else{ document.getElementById(nodename).setAttribute("noadd","true");}
